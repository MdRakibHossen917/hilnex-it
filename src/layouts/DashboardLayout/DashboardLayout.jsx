...
const dropdownMenuItems = [
  { id: 1, name: 'My Profile', icon: <FiUser size={16} />, path: '/dashboard/profile' },
  { id: 2, name: 'Logout', icon: <FiLogOut size={16} />, action: logout } 
];
  ...
  const getPriorityColor = (priority) => {
    switch (priority) {
      case 'Super Express': return '#ef4444';
      case 'Express': return '#f59e0b';
      case 'Regular': return '#00ff88';
      default: return '#6b7280';
    }
  };
  ...
  // Function to generate line path for SVG
  const generateLinePath = () => {
    if (revenueData.length === 0) return '';
    
    const points = revenueData.map((item, index) => {
      const x = 40 + (index / (revenueData.length - 1)) * 320;
      const y = 160 - (item.revenue / 80) * 140;
  ...
  const handleDropdownItemClick = (item) => {
    if (item.action) {
      item.action(); // If it's logout action
    } else if (item.path) {
      navigate(item.path); // If it's a path
    }
            ...
            <div className="header-actions">
              ...
              >
                <div className="user-avatar">
                  {/* If user has image, show image, else show user icon */}
                  {user?.avatar ? (
                    <img 
                      src={user.avatar} 
                      alt="User Avatar" 
                ...
                <div className="user-info">
                  <span className="user-name">
                    {user?.name || 'User'} {/* Show logged in user's name */}
                  </span>
                ...
                <FiChevronDown 
                  className={`dropdown-arrow ${showDropdown ? 'rotate-180' : ''}`} 
                  size={16} 
                />
                ...
                {showDropdown && (
                  <div className="dropdown-menu">
                    {dropdownMenuItems.map(item => (
                      <div
                        key={item.id}
                        className={`dropdown-item ${item.name === 'Logout' ? 'logout-item' : ''}`}
        ...
        <main className="dashboard-main">
          <div className="page-header">
            ...
            <p className="page-subtitle">
              Welcome back, {user?.name || 'User'}! {/* Show user name in welcome message */}
            </p>
          </div>
            ...
            {isDashboardHome ? (
              <div className="dashboard-overview">
                ...
                <div className="charts-row">
                  ...
                  {/* Revenue Growth Line Chart - SVG Version */}
                  <div className="chart-card">
                    <div className="chart-header">
                      ...
                      <div className="line-chart-svg-container">
                        ...
                        >
                          ...
                          {revenueData.map((_, index) => (
                            <line 
                              key={`v-grid-${index}`}
                              x1={40 + (index / (revenueData.length - 1)) * 320}
                              y1="20"
                              x2={40 + (index / (revenueData.length - 1)) * 320}
                              y2="180"
                              stroke="#2a2a2a"
                              strokeWidth="1"
                              strokeDasharray="2,2"
                          ...
                          {/* Y-axis labels */}
                          <text x="30" y="25" textAnchor="end" fill="#666" fontSize="10" fontWeight="600">$80K</text>
                          <text x="30" y="65" textAnchor="end" fill="#666" fontSize="10" fontWeight="600">$60K</text>
                          <text x="30" y="105" textAnchor="end" fill="#666" fontSize="10" fontWeight="600">$40K</text>
                          <text x="30" y="145" textAnchor="end" fill="#666" fontSize="10" fontWeight="600">$20K</text>
                          <text x="30" y="185" textAnchor="end" fill="#666" fontSize="10" fontWeight="600">$0K</text>
                          ...
                          {/* Area under line */}
                          <path
                            d={`${generateLinePath()} L ${40 + ((revenueData.length - 1) / (revenueData.length - 1)) * 320} 180 L 40 180 Z`}
                          ...
                          <defs>
                            <linearGradient id="lineGradient" x1="0%" y1="0%" x2="100%" y2="0%">
                              <stop offset="0%" stopColor="#ff8402" />
                              <stop offset="100%" stopColor="#ffb347" />
                            </linearGradient>
                            <linearGradient id="areaGradient" x1="0%" y1="0%" x2="0%" y2="100%">
                              <stop offset="0%" stopColor="#ff8402" stopOpacity="0.4" />
                              <stop offset="100%" stopColor="#ff8402" stopOpacity="0.1" />
                            </linearGradient>
                          </defs>
                        </svg>
                      </div>
